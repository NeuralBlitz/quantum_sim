...
class CircuitDrawer:
    ...
    def draw(self, circuit: "QuantumCircuit", filename: str = None):
        ...
        """
        # 1. Coordinate Setup
        # Create initial coordinates to determine the boundary of the figure
        ...
        # Determine figure width based on the furthest gate
        max_x = 5.0
        if info_pass:
    ...
    def _draw_box(self, ax, item):
        colors = {
            'sub_circuit_box': ('#FFF9C4', '#FBC02D'),  # Yellow
            'layer_box': ('#FFEBEE', '#EF5350'),        # Red
            'block_box': ('#E3F2FD', '#42A5F5')         # Blue
        }
